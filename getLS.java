...
public class getLS {
    ...
    public static byte[] getLS(InetAddress destIP, Constantes flagPort) throws NotConnectedExcpetion {
        try {
            ...
            DatagramPacket synLS = packetOfSynLs(receiveSocket.getLocalPort(), destIP, flagPort);
                ...
                try {
                 ...
                 //   sendSocket.send(synLS);
                    if (receiveFileInfo[0] == (byte) Constantes.ACKLs) {
                        ...
                        endConnection(sendSocket, receiveSocket, destIP, flagPort, finLS);
    ...
    private static void endConnection(DatagramSocket sendSocket, DatagramSocket receiveSocket,
                                      InetAddress destIP, Constantes flagPort, DatagramPacket finLS) {
        try {
            int attempts = 10;
            boolean finACK = false;
            byte[] lastFinB = new byte[1];
            lastFinB[0] = (byte) Constantes.FynAck;
            DatagramPacket ackPacket = new DatagramPacket(lastFinB, lastFinB.length, destIP, flagPort.WhatPortToSend());
